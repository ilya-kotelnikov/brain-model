...
MNISTDataset::MNISTDataset(const wchar_t* ubyte_idx_filepath)
    : ubyte_idx_file_(nullptr), data_count_(0), data_size_(0) {
  errno_t res = _wfopen_s(&ubyte_idx_file_, ubyte_idx_filepath, L"rb");
  if (res != 0)
    throw std::runtime_error("MNISTDataset can't open the ubyte idx file");
  ...
  if (read_count != 1)
    throw std::runtime_error("MNISTDataset can't read the ubyte idx file's header");
  if (idx_file_header.zero != 0 || idx_file_header.data_type_code != 8 ||
      idx_file_header.dimension_count == 0) {
    throw std::runtime_error("MNISTDataset ubyte idx file's header is invalid");
  }
...
bool MNISTDataset::ReadNext() {
  ...
  if (read_count != data_size_) {
    throw std::runtime_error(
        "MNISTDataset the ubyte idx file has invalid data");
  }
  return true;
}

uint32_t MNISTDataset::GetDataSize() const {
  return data_size_;
}

const uint8_t* MNISTDataset::GetDataBuffer() const {
  return &data_read_next_.front();
}

const DatasetValueCast& MNISTDataset::GetDataValueCast() const {
  return data_value_cast_;
}

uint32_t MNISTDataset::ReadBigEndeanUint32() {
  ...
  if (read_count != uint32_bytes.size()) {
    throw std::runtime_error(
        "MNISTDataset the ubyte idx file has invalid format");
  }
